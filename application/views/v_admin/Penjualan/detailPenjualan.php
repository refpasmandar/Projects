    ...
    <div class="row">
        <div class="col-lg-12">
            <div class='text-right btnBack'>
                <a class="btn btn-sm btn-warning" href="<?php echo base_url('Penjualan/Penjualan/daftarPenjualan'); ?>"><i class="fas fa-arrow-circle-left"></i> Kembali</a>
            </div>
            <div class="card">
                <div class="card-header">
                    <h6 class="text-primary font-weight-bold"> Daftar Invoice
                        <small class="text-danger">
                            <?php foreach ($namaCust as $t) : ?>
                                <?= $t->nama_customer ?>
                            <?php endforeach; ?>
                        </small>
                    </h6>
                            ...
                            <tbody class="text-center">
                                ...
                                <?php foreach ($testing as $t) : ?>
                                    <tr>
                                        ...
                                        <td> Rp <?= substr(number_format($t->totalTransaksi, 0, ',', '.'), 1, 12) ?> <span class="text-primary">
                                        <td>Rp <?= substr(number_format($t->totalBayar, 0, ',', '.'), 0, 12) ?></td>
                                        <?php if ($t->totalPiutang == 0) { ?>
                                            <td> Rp <?= substr(number_format($t->totalPiutang, 0, ',', '.'), 0, 12) ?> <span class="text-primary">(Lunas)</span></td>
                                        <?php } else { ?>
                                            <td> Rp <?= substr(number_format($t->totalPiutang, 0, ',', '.'), 0, 12) ?></td>
                                        <?php } ?>
                                        <td class="text-center">
                                            <a href="<?= base_url(); ?>Penjualan/Penjualan/detailTransaksiPenjualan/<?= $t->kode_jurnal . '/' . $t->tanggal_terbaru; ?>" class="btn btn-info"><i class="fas fa-file-invoice-dollar"></i> Detail</a></a>
                                        </td>
                                    </tr>
                                <?php endforeach ?>
                            </tbody>
