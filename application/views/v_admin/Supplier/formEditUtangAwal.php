    ...
    <div class="card">
        <div class="card-header">
            <h6 class="text-primary font-weight-bold"><i class="fas fa-cart-arrow-down mr-2"></i>
                Form Edit Utang Awal
            </h6>
        </div>
        <div class="card-body">
            <?php $this->view('flashdata') ?>
            <form action="<?php echo base_url('Master_data/Supplier/prosesEditUtangAwal') ?>" method="post">
                <div class="row">
                    <div class="col-lg-6">
                        <div class="card-header">
                            <?php foreach ($jurnal as $jur) : ?>
                                <?php if ($jur->jenis_transaksi == 'Utang Awal') { ?>
                                    <input type="hidden" name="id_jurnal[]" id="id_jurnal" value="<?= $jur->id_jurnal ?>">
                                <?php } ?>
                            <?php endforeach; ?>
                            <?php foreach ($single as $s) : ?>
                                <?php if ($s->jenis_transaksi == 'Utang Awal') {  ?>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="form-group">
                                                        ...
                                                        <input class="form-control total_awal" name="total_awal" id="total_awal<?php echo $no ?>" type="hidden" value="<?php echo $lb->total_beli ?>">
                                                    </div>
                                                </td>
                                                <td hidden>
...
<script>
    // di load ketika halaman diakses
    ...
    $(document).on('keyup keydown click change', function() {
        ...
        var grand_total = 0;
        var batasUtang = 0;
        var saldoUtang = parseInt($("#saldoUtang").val());
        var diskon_total = parseInt($("#diskon_total").val());
        var jumlah_bayar = parseInt($("#jumlah_bayar").val());
        $('.total').each(function() {
            grand_total += +$(this).val();
        ...
        $('#batasUtang').val(saldoUtang - ((grand_total - jumlah_bayar) - batasUtang));
